...
public class MindLayerUtil {
    ...
    public static <T extends IArea, C extends IExtendedNoiseRandom<T>> ImmutableList<IAreaFactory<T>> buildOverworldProcedure(WorldType worldTypeIn, MindGenSettings settings, LongFunction<C> contextFactory) {
        ...
        for (int k = 0; k < i; ++k) {
            ...
            if (k == 1 || i == 1) {
                lvt_8_1_ = ZoomLayer.NORMAL.apply((IExtendedNoiseRandom) contextFactory.apply(1000L), lvt_8_1_);
            }
        }
    ...
    public static Layer[] buildOverworldProcedure(long seed, WorldType typeIn, MindGenSettings settings) {
        int[] aint = new int[1];
